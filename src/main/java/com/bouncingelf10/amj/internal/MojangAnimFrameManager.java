...
public class MojangAnimFrameManager {
    public static final AnimationTimeline timeline = AnimatedMojangLogoClient.getClientAnimationManager().createTimeline("mojang_logo");
    ...
    public static void start() {
        ...
        timeline.onFinish(() -> {
            hasFinished = true;
            AnimatedMojangLogoClient.getClientCountdownManager().startClient(Duration.SECOND).onFinish(client -> stop());
        });
        timeline.play();

        AnimatedMojangLogoClient.LOGGER.info("Playing sound and starting Mojang logo animation");
        Minecraft.getInstance().getSoundManager().play(
                SimpleSoundInstance.forUI(ModSounds.STARTUP.get(), 1.0f)
        );
    }
